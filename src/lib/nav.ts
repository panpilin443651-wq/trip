import type { NavIconName } from "@/components/NavIcon";

export interface NavItem {
  href: string;
  label: string;
  /** ไอคอนเส้นในแถบเมนู ใช้แทนอิโมจิเพื่อให้เปลี่ยนสีตามธีมได้ */
  icon: NavIconName;
  /** แสดงบนแถบล่างของมือถือหรือไม่ */
  primary: boolean;
  description?: string;
}

/**
 * เมนูทั้งหมดของแอป
 *
 * สี่อันแรกเป็นแถบล่างบนมือถือ เรียงตามลำดับที่ใช้จริงตอนวางแผนทริป
 * ดูว่าอยู่ตรงไหน (แผนที่) → ลงแผน (แผนเที่ยว) → ถามเมื่อคิดไม่ออก (ผู้ช่วย)
 * โดยมีหน้าหลักเป็นจุดตั้งต้น
 *
 * แนะนำเที่ยวไม่ได้เป็นเมนูของตัวเองแล้ว ย้ายไปเป็นแท็บในหน้าแผนเที่ยว
 * เพราะการเลือกที่เที่ยวกับการจัดลงวันเป็นงานเดียวกัน สลับไปมาบ่อย
 *
 * โปรไฟล์ไปอยู่เป็นปุ่มมุมขวาบนแทน เพราะไม่ใช่ปลายทางที่คนเข้าบ่อยเท่าสี่เมนูนี้
 * แถบล่างจึงเหลือสี่ช่อง ปุ่มใหญ่ขึ้นและกดถูกง่ายขึ้น
 */
export const NAV_ITEMS: NavItem[] = [
  {
    href: "/dashboard",
    label: "หน้าหลัก",
    icon: "home",
    primary: true,
    description: "ภาพรวมทริปทั้งหมด",
  },
  {
    href: "/map",
    label: "แผนที่",
    icon: "map",
    primary: true,
    description: "เส้นทางและระยะทาง",
  },
  {
    href: "/settings",
    label: "แผนเที่ยว",
    icon: "suitcase",
    primary: true,
    description: "ข้อมูลทริป แผนรายวัน จังหวัด และงบรวม",
  },
  {
    href: "/chat",
    label: "ผู้ช่วย",
    icon: "chat",
    primary: true,
    description: "ถามว่าไปเที่ยวไหนดี วิธีใช้เว็บ และเรื่องทริปของคุณ",
  },
  {
    // อยู่มุมขวาบน ไม่ได้อยู่บนแถบล่าง แต่ยังอยู่ในแถบข้างของจอใหญ่
    href: "/more",
    label: "โปรไฟล์",
    icon: "user",
    primary: false,
    description: "บัญชี งบประมาณ สรุปแผน และเมนูที่เหลือ",
  },
  {
    href: "/budget",
    label: "งบประมาณ",
    icon: "wallet",
    primary: false,
    description: "ค่าใช้จ่ายแยกหมวด",
  },
  {
    href: "/summary",
    label: "สรุปแผน",
    icon: "document",
    primary: false,
    description: "ดูแผนทั้งทริป บันทึกเป็นรูปหรือ PDF",
  },
];

export const PRIMARY_NAV = NAV_ITEMS.filter((item) => item.primary);

/**
 * เมนูของแถบข้างบนจอใหญ่
 *
 * ตัดสองอันที่มีทางเข้าอื่นอยู่แล้วออก จะได้ไม่มีสองทางเข้าไปที่เดียวกัน
 * ซึ่งทำให้คนลังเลว่าต่างกันยังไง
 *
 *   /more     โปรไฟล์ — อยู่ที่ปุ่มมุมขวาบน
 *   /settings แผนเที่ยว — เข้าจากชื่อทริปด้านบนของแถบข้าง และจากหน้าหลัก
 */
const SIDEBAR_HIDDEN = new Set(["/more", "/settings"]);

export const SIDEBAR_NAV = NAV_ITEMS.filter(
  (item) => !SIDEBAR_HIDDEN.has(item.href),
);

/**
 * เมนูที่ไม่ได้อยู่บนแถบล่าง — ไปรวมกันในหน้าโปรไฟล์
 *
 * ตัดหน้าโปรไฟล์เองออก ไม่งั้นหน้านั้นจะมีลิงก์ชี้กลับมาที่ตัวเอง
 */
export const SECONDARY_NAV = NAV_ITEMS.filter(
  (item) => !item.primary && item.href !== "/more",
);
